...
use stream_engine::SessionHandle;
...
pub async fn run_microphone(session: &SessionHandle, queue_len: usize) -> anyhow::Result<()> {
...
pub async fn run_file(session: &SessionHandle, path: &Path, realtime: bool) -> anyhow::Result<()> {
    ...
    for chunk in pcm.chunks(chunk_samples) {
        session.push_pcm(chunk.to_vec()).await;
        if realtime {
